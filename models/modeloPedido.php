...
class ModeloPedido
{
    ...
    {
        $sql = "SELECT * FROM $this->tabla INNER JOIN mesa ON mesa.id_mesa = pedido.id_mesa INNER JOIN usuario ON usuario.id_usuario = pedido.id_usuario INNER JOIN estado_mesa ON estado_mesa.id_estado_mesa = pedido.id_estado_mesa WHERE pedido.id_mesa = ? AND pedido.fecha_ingreso = ? AND pago = 0";
        $conn = new Conexion();
        $stms = $conn->conectar()->prepare($sql);
    ...
    {
        $sql = "UPDATE $this->tabla SET id_mesa = ?, id_estado_mesa = ? WHERE id_mesa = ? AND fecha_ingreso = ?";
        $conn = new Conexion();
        $stms = $conn->conectar()->prepare($sql);
        $stms->bindParam(1, $id_mesa, PDO::PARAM_INT);
    ...
    {
        $sql = "SELECT MAX(fecha_ingreso), id_mesa FROM $this->tabla WHERE print = ? AND id_mesa = ?";
        $conn = new Conexion();
        $stms = $conn->conectar()->prepare($sql);
        $stms->bindParam(1, $print, PDO::PARAM_INT);
        $stms->bindParam(2, $id, PDO::PARAM_INT);
    ...
    {
        $sql = "SELECT * FROM $this->tabla INNER JOIN producto ON producto.id_producto = pedido.id_producto INNER JOIN categoria ON categoria.id_categoria = producto.id_categoria WHERE fecha_ingreso = ? AND print = ?";
        $conn = new Conexion();
        $stms = $conn->conectar()->prepare($sql);
        $stms->bindParam(1, $fecha, PDO::PARAM_STR);
    ...
    function listarPedidoFacturaModelo($id)
    {
        date_default_timezone_set('America/Mexico_City');
        $fechaActal = date('Y-m-d');
        $sql = "SELECT * FROM $this->tabla INNER JOIN producto ON producto.id_producto = pedido.id_producto WHERE id_mesa = ? AND fecha_ingreso LIKE ? AND id_estado_mesa = 4 AND pago = 0";
        $conn = new Conexion();
        $stms = $conn->conectar()->prepare($sql);
        $fechaActal = "%" . $fechaActal . "%";
        $stms->bindParam(1, $id, PDO::PARAM_INT);
    ...
    {
        $sql = "UPDATE $this->tabla SET pago = 1 WHERE fecha_ingreso = ? AND id_producto = ?";
        $conn = new Conexion();
        $stms = $conn->conectar()->prepare($sql);
        $stms->bindParam(1, $fecha, PDO::PARAM_STR);
        $stms->bindParam(2, $id, PDO::PARAM_INT);
