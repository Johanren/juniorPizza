...
class ModeloProeevedor
{
    ...
    {
        $sql = "UPDATE $this->tabla SET nit_proeevedor=?,nombre_proeevedor=?,telefono_proeevedor=?,direccion_proeevedor=?,id_local=? WHERE id_proeevedor=?";
        $conn = new Conexion();
        $stms = $conn->conectar()->prepare($sql);
        if ($dato != '') {
            $stms->bindParam(1, $dato['nit'], PDO::PARAM_STR);
    ...
    function eliminarProeevedorIdModelo($id)
    {
        ...
        try {
            ...
            $stms = $conn->conectar()->prepare($sql);
            if ($stms->execute()) {
                $sql = "DELETE FROM $this->tabla WHERE id_proeevedor = ?";
