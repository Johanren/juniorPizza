...
class ModeloVenta
{
    ...
    function informeVentainicioFinFacturaModelo($inicio,$fin){
        $sql = "SELECT DISTINCT MAX(factura.id_factura), MIN(factura.id_factura) FROM venta INNER JOIN producto ON producto.id_producto = venta.id_producto INNER JOIN factura ON factura.id_factura = venta.id_factura INNER JOIN impuesto ON producto.id_impuesto = impuesto.id_impuesto WHERE factura.factura = 'true' AND venta.fecha_ingreso BETWEEN ? AND ? AND factura.id_local = ? GROUP BY producto.id_producto, producto.nombre_producto, impuesto.numero_impuesto, impuesto.nombre_impusto, impuesto.base_impuesto";
